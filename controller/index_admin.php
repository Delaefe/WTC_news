...
switch ($action) {
	...
	if ($title == NULL || $description == NULL || $topic == NULL || $topic == false || $content == NULL || $img == NULL || $date == NULL || $author == NULL) {
		...
		header("Location: index_admin.php?topic_id=$topic");
	}		
	...
	if ($title == NULL || $description == NULL || $topic == NULL || $topic == false || $content == NULL || $date == NULL) {
		...
		header("Location: index_admin.php?action=show_article&new_id=$new_id");
	}		
	...
	foreach ($topics as $topic) {
		$stadistics[] = get_count_for_topic($topic['topicId']);
	}
	...
	if ($article['frontpage'] == 0) {
		switch_frontpage($new_id, 1);
	}
	elseif ($article['frontpage'] == 1){
		switch_frontpage($new_id, 0);
	}
